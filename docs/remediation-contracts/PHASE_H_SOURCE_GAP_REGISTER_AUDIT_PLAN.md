# Phase H — Global Source-Gap Register Audit Plan

**Status:** FINAL / REVIEWED / ADJUDICATED / GOVERNING PHASE-H PLAN  
...
13. whether the gap is node-local or a shared cross-node gap.
...
### 3.1 Mandatory two-layer semantic-vs-naming discriminator

For every candidate taxonomy/state/phase/class gap, ask this question explicitly:

> **Is the uncertainty about whether the recalled distinctions themselves are correct and complete, or only about what to call already-confirmed distinctions?**

Classify as follows:

- if the set of distinctions, their boundaries, completeness, or normative behavior may itself be wrong or incomplete, classify `H_SEMANTIC_IMPLEMENTATION` (and also `H_NAMING_EXACTNESS` if the labels are independently uncertain);
- if the semantic model is already confirmed and only the historical label/string/name is uncertain, classify `H_NAMING_EXACTNESS` only.

This test prevents a recalled but unverified semantic taxonomy from being mislabeled as mere naming debt.

Calibration:

- **R8 reconciliation-capability taxonomy:** semantic + naming uncertainty because even the five recalled distinctions are not source-certified as the correct/complete model.
- **R20 three-phase model:** naming-only uncertainty because the three distinct phase semantics are confirmed and only exact literal strings remain source-unresolved.
...
### 6.1 Cross-node overlap / reverse-deduplication rule

The register must also guard against the reverse counting error: the **same underlying missing proposition appearing independently in multiple node artifacts**.

Before finalizing any candidate `H_SEMANTIC_IMPLEMENTATION` gap as node-local, Phase H must check whether another node:

- explicitly references the same missing proposition;
- depends on the same unrecovered taxonomy/state model/authority distinction;
- independently recalls the same missing source rule;
- or can only be implemented safely by resolving the same underlying source question.

If yes, register **one shared primary gap** with multiple affected nodes rather than duplicating node-local gaps.

Do not merge merely because two gaps have similar wording. Shared-gap status requires semantic identity of the missing proposition.

This rule follows the prior global-audit precedent where repeated source-exhausted categories were recognized as one pattern across R4/R5/R6 rather than treated as unrelated local accidents.
...
Reason: the uncertainty is not merely what to call five confirmed states. The current record does not establish that the recalled five distinctions are the correct and complete semantic model. That uncertainty can affect which reconciliation/replay behavior is safe.
...
The semantic phase model itself is settled, so this fails the semantic-gap half of the two-layer test and remains naming-only.

### R8 × R16 overlap candidate

R8's unrecovered reconciliation-capability taxonomy is the first mandatory cross-node overlap test.

Initial repository searches for the exact phrase `reconciliation capability taxonomy` and the recalled candidate label `RECONCILABLE_BY_RUN_ID` returned no indexed duplicate outside the R8 context. That result is **not** proof the gap is R8-local and is **not** source-exhaustion evidence.

H0/H1 must inspect R16 semantically because R16 owns deterministic financial reconciliation. The question is whether R16's own normative behavior depends on the same missing capability/replay taxonomy even if it never repeats R8's exact terminology. If so, H must register one shared R8/R16 gap; if not, the R8 gap remains node-local.
...
H0 must also mark candidate cross-node overlaps whenever two or more nodes appear to depend on the same missing proposition.
...
- does another node reference, depend on, or independently recall the same missing proposition?

### H1.5 — cross-node overlap adjudication

Before assigning final semantic-gap counts:

1. take every candidate semantic/blocking gap;
2. inspect explicit cross-references and neighboring contracts for the same missing proposition;
3. distinguish semantic identity from merely similar vocabulary;
4. merge genuine shared gaps into one primary entry with all affected nodes;
5. preserve node-local entries where the missing propositions are independently recoverable/adjudicable;
6. record the evidence for merging or refusing to merge.

The R8/R16 reconciliation-taxonomy candidate is the first required H1.5 test.
...
- no duplicate counting of one missing proposition across multiple nodes.
...
10. every semantic/blocking candidate has undergone cross-node overlap review;
11. shared missing propositions are counted once with all affected nodes;
12. adversarial review pressure-tests the semantic/blocking classifications, overlap decisions, and arithmetic.
