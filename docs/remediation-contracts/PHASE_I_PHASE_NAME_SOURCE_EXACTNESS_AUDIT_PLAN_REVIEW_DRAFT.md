# Phase I — Phase / Name / Source Exactness Audit Plan — Review Draft
...
## 6. Phase-I batch sequence

### I0 — nine-item exactness inventory

Confirm all 9 handoff items, their node/source blobs, semantic dependency, Phase-H source state, and candidate names.
...
- R20's three literal phase strings are one primary exactness item because they are one three-phase naming proposition;
...
Do not split one item merely because it contains several literals unless those literals are independently recoverable and serve distinct semantic naming surfaces.
...
2. no naming-only uncertainty is mislabeled semantic uncertainty;
3. no semantic uncertainty is hidden behind a naming-only disposition;
4. every exact-name claim names its source basis;
5. candidate/reconstructed labels are not promoted to historical fact without source;
6. governed replacement labels, if adopted later, are explicitly non-historical;
7. R8 dual-layer dependency is preserved;
8. R20 three-phase semantics remain fixed regardless of literal-name outcome;
9. R20 Phase-I naming and Phase-J enforcement ownership remain distinct;
10. Phase-H source states are not silently upgraded;
11. adversarial review is complete;
12. implementation authority remains suspended pending the frozen global sequence.

## 9. Review questions

Adversarial review should pressure especially:

1. Is R8 correctly separated as the sole item whose naming question depends on unresolved semantic taxonomy completeness?
2. Are any of I-02 through I-09 secretly semantic rather than naming-only?
3. Are any current labels actually source-confirmed strongly enough to retire from Phase I now?
4. Does grouping R20's three literal phase strings as one item still satisfy independent recoverability/counting discipline?
5. Does any Phase-I item improperly include ordinary schema/storage field-name exactness that should remain only in H2?
6. Is the R20 E44 naming / E45 mechanism / Phase-J coverage boundary stated strongly enough to prevent ownership collapse?
7. Are any additional Phase-I items missing from H4's 9-item handoff?

## 10. Provisional result

`PHASE I PLAN READY / 9-ITEM HANDOFF FIXED / 8 PROVISIONAL NAMING-ONLY ITEMS / 1 DUAL-LAYER R8 DEPENDENCY / NO EXACT HISTORICAL NAME PROMOTION WITHOUT SOURCE`
