...
export function Layout({
  ...
  const needsYouQuery = useQuery<HumanActionsCountResponse>({
    ...
    queryFn: async () => {
      const response = await fetch("/api/human-actions?status=ACTIVE", {
        credentials: "include",
      });
      if (!response.ok)
        throw new Error(`Human actions returned ${response.status}`);
  ...
  const criticalCount =
    needsYouQuery.data?.actions?.filter((item) => item.urgency === "CRITICAL")
      .length ?? 0;
  const highCount =
    needsYouQuery.data?.actions?.filter((item) => item.urgency === "HIGH")
      .length ?? 0;
  ...
  const navItems = [
    ...
    { href: "/factory", label: "Asset Factory", icon: Boxes },
    ...
    <div className="min-h-[100dvh] flex flex-col md:flex-row bg-muted/30">
              ...
              >
                ...
                {typeof item.count === "number" && item.count > 0 && (
                  <span
                    ...
                    title={
                      item.urgentCount && item.urgentCount > 0
                        ? `${item.urgentCount} high-urgency actions`
                        : `${item.count} open actions`
                    }
                  >
                    {item.count}
                  </span>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        {criticalCount > 0 ? (
          <Link
            href="/needs-you"
            className="flex items-center justify-center gap-2 border-b border-red-300 bg-red-600 px-4 py-2 text-xs font-semibold text-white hover:bg-red-700"
          >
            <BellRing className="h-4 w-4" />
            {criticalCount} critical human action
            {criticalCount === 1 ? "" : "s"} require immediate attention
          </Link>
        ) : highCount > 0 ? (
          <Link
            href="/needs-you"
            className="flex items-center justify-center gap-2 border-b border-orange-300 bg-orange-50 px-4 py-2 text-xs font-semibold text-orange-950 hover:bg-orange-100"
          >
            <BellRing className="h-4 w-4" />
            {highCount} high-priority human action{highCount === 1 ? "" : "s"}{" "}
            currently block workflow progress
          </Link>
        ) : null}
