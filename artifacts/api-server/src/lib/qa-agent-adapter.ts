...
export type QaCostMode = "ZERO_CASH" | "METERED" | "UNKNOWN";
...
function state(value: unknown): QaRunState {
  ...
  if (
    ["QUEUED", "RUNNING", "PASSED", "FAILED", "BLOCKED", "CANCELLED"].includes(
      normalized,
    )
  ) {
  ...
  throw new Error(
    `QA_ADAPTER_INVALID_RESPONSE: unsupported state ${String(value)}`,
  );
}
...
function acceptanceResults(value: unknown): PersistedQaAcceptanceResult[] {
  ...
  return value.flatMap((item) => {
    ...
    if (!criterion || !["PASS", "FAIL", "UNKNOWN"].includes(rawStatus))
      return [];
    return [
      {
...
function defects(value: unknown): PersistedQaDefect[] {
  ...
  return value.flatMap((item, index) => {
    ...
    return [
      {
        key: (
          text(object.key ?? object.code) ?? `QA_DEFECT_${index + 1}`
        ).slice(0, 160),
        category: (text(object.category) ?? "UNKNOWN")
          .toUpperCase()
          .slice(0, 120),
        severity,
        ...
        evidence: text(object.evidence)?.slice(0, 4_000) ?? null,
        repairGuidance:
          text(object.repair_guidance ?? object.repairGuidance)?.slice(
            0,
            4_000,
          ) ?? null,
...
function parseResult(value: unknown): QaResult {
  ...
  if (!providerRunId)
    throw new Error("QA_ADAPTER_INVALID_RESPONSE: provider run id is required");
  ...
  return {
    ...
    acceptanceResults: acceptanceResults(
      object.acceptance_results ?? object.acceptanceResults,
    ),
    defects: defects(object.defects),
    baselineChecksPassed: bool(
      object.baseline_checks_passed ?? object.baselineChecksPassed,
    ),
    requiresHumanAction:
      object.requires_human_action === true ||
      object.requiresHumanAction === true,
    humanAction:
      object.human_action &&
      typeof object.human_action === "object" &&
      !Array.isArray(object.human_action)
...
export function configuredQaAdapter(): QaAgentAdapter | null {
  ...
  const provider =
    process.env.MONEY_SCOUT_QA_ADAPTER_PROVIDER?.trim() || "GENERIC_HTTP_QA";
  const rawCost =
    process.env.MONEY_SCOUT_QA_ADAPTER_COST_MODE?.trim().toUpperCase();
  const costMode: QaCostMode =
    rawCost === "METERED"
      ? "METERED"
      : rawCost === "ZERO_CASH"
        ? "ZERO_CASH"
...
export function createHttpQaAdapter(config: QaAdapterConfig): QaAgentAdapter {
  ...
  const parseHttpResult = async (
    ...
    if (!response.ok)
      throw new Error(
        `QA_ADAPTER_HTTP_${response.status}: ${operation}: ${raw.slice(0, 1_000)}`,
      );
    let parsed: unknown;
    try {
      ...
      throw new Error(
        `QA_ADAPTER_INVALID_RESPONSE: ${operation} response was not JSON`,
      );
    }
  ...
  return {
      ...
      const response = await fetch(`${config.baseUrl}/v1/qa-runs`, {
        ...
        body: JSON.stringify({
          ...
          independence_requirement:
            "Do not trust the builder's success claim. Verify executable evidence independently.",
          autonomy: {
    ...
    async getStatus(providerRunId) {
      const response = await fetch(
        `${config.baseUrl}/v1/qa-runs/${encodeURIComponent(providerRunId)}`,
        {
          method: "GET",
          headers: headers(),
