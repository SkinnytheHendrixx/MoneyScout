...
import { pool, prepareBetCapitalAllocationSchema, prepareRuntimeSchema, REQUIRED_BET_RUNTIME_TABLES, REQUIRED_RUNTIME_TABLES } from "@workspace/db";
...
assert.equal(betSecond.appliedMigrationIds.length, 0, "Bet runtime migration must be idempotent");
const betTables = await pool.query<{ table_name: string }>(
  "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = ANY($1::text[])",
  [[...REQUIRED_BET_RUNTIME_TABLES]],
);
assert.deepEqual(new Set(betTables.rows.map((row) => row.table_name)), new Set(REQUIRED_BET_RUNTIME_TABLES));
...
assert.equal(legacyLink.rows[0]?.is_nullable, "YES", "legacy Builds must remain valid without fabricated Bet linkage");
